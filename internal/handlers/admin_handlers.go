...
func AdminDeleteTournament(c *gin.Context) {
	...
	// Активный турнир хранится в Redis (см. SetActiveTournament), а не в
	// Postgres — там появляется запись только после его автоматического
	// завершения через TournamentWatcher. Поэтому раньше удаление текущего
	// турнира через этот эндпоинт молча ничего не делало (0 строк в
	// Postgres, но ответ "успех"). Сначала проверяем и чистим Redis.
	if active, err := service.GetActiveTournament(); err == nil && active.ID == id {
		if err := database.RDB.Del(
			database.Ctx,
			"tournament:active",
	...
	result := database.DB.Delete(&models.Tournament{}, id)
